...
CREATE TRIGGER PSUTRIG AFTER UPDATE ON PRODUCT_SOLD 
FOR EACH ROW  
BEGIN 
UPDATE PRODUCT_SOLD SET TOTAL_PRICE = NEW.QUANTITY*(SELECT PRICE_PER_UNIT FROM PRODUCT WHERE PID=NEW.PID AND DID=NEW.DID AND BID=NEW.BID AND CID=NEW.CID);
END;$$

CREATE TRIGGER PSITRIG AFTER INSERT ON PRODUCT_SOLD 
FOR EACH ROW  
BEGIN 
UPDATE PRODUCT_SOLD SET TOTAL_PRICE = NEW.QUANTITY*(SELECT PRICE_PER_UNIT FROM PRODUCT WHERE PID=NEW.PID AND DID=NEW.DID AND BID=NEW.BID AND CID=NEW.CID);
END;$$
...
BEGIN 
SELECT PID,BID,DID,CID  FROM PRODUCT WHERE EXP_DATE = CURDATE(); 
END$$

CREATE PROCEDURE EXP_DEC () 
BEGIN 
UPDATE PRODUCT SET PRICE_PER_UNIT=PRICE_PER_UNIT*0.8 WHERE EXP_DATE = CURDATE();
END$$
