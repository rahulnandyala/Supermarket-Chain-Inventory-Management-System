...
BEGIN 
SELECT PRODUCT_NAME,PID,BID,DID FROM PRODUCT WHERE EXP_DATE = CURDATE() AND CID = COMPID; 
END$$
...
CREATE PROCEDURE EXP_DECCHK (IN COMPID INT) 
BEGIN 
SELECT PRODUCT_NAME, QUANTITY, PRICE_PER_UNIT, PRICE_PER_UNIT*0.8 AS NEW_PRICE FROM PRODUCT WHERE CID = COMPID AND EXP_DATE < DATE_ADD(CURDATE(), INTERVAL 10 DAY);
END$$
